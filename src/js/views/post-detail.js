    ...
    '<article class="comment" data-comment-id="' + escape(c.id) + '">' +
      ...
      '<div class="comment__main">' +
        '<div class="comment__head">' +
          ...
          (isMine
            ? ("<button class=\"comment__delete\" title=\"" + t("削除") + "\" data-comment-delete=\"") + escape(c.id) + '">' +
  ...
  if (!me) {
    return (
      '<div class="comment-gate">' +
        ("<button class=\"btn btn--primary\" data-auth=\"login\">" + t("サインインしてコメントする") + "</button>") +
      '</div>'
    );
  }
  return (
    '<form class="comment-form" id="comment-form">' +
      ...
      '<div class="comment-form__main">' +
        ("<textarea name=\"body\" rows=\"2\" maxlength=\"500\" placeholder=\"" + t("コメントを書く…") + "\"></textarea>") +
        '<div class="comment-form__actions">' +
          ("<button type=\"submit\" class=\"btn btn--primary btn--sm\">" + t("送信") + "</button>") +
        '</div>' +
      '</div>' +
    '</form>'
  );
...
export async function hydratePostDetail(id) {
  ...
  const analyticsLink = isOwn
    ? '<a class="post-detail__analytics" href="' + url('/post/' + post.id + '/analytics') + '">' +
        icon('chart', { size: 14, className: 'icon--inline' }) + t("アクティビティを見る") +
      '</a>'
    : '';
  ...
  root.innerHTML =
    '<a class="back-home" href="/">' + icon('reply', { size: 14, className: 'icon--inline' }) + (t("Home に戻る") + "</a>") +
    '<div id="post-detail-card">' + renderPost(post) + '</div>' +
    (analyticsLink ? '<div class="post-detail__bar">' + analyticsLink + '</div>' : '') +
    ("<div class=\"comment-list-head\"><h3>" + t("コメント") + "</h3></div>") +
    commentComposer(me) +
    ("<div class=\"comment-list\" id=\"comment-list\"><div class=\"stub\"><p class=\"stub__sub\">" + t("コメントを読み込み中…") + "</p></div></div>");
  ...
  const reactionsPromise = withTimeout(Promise.all([
    ...
    ]), DETAIL_TIMEOUT_MS, t("リアクション取得")).catch(() => null);
  const commentsPromise = withTimeout(getComments(id), DETAIL_TIMEOUT_MS, t("コメント取得"))
  ...
  if (head) head.innerHTML = ("<h3>" + t("コメント ") + "<span class=\"dim\">(") + comments.length + ')</span></h3>';
  const list = document.getElementById('comment-list');
  if (list) list.innerHTML =
      (comments.length
        ? comments.map(c => renderComment(c, me)).join('')
        : ("<div class=\"stub\"><p class=\"stub__sub\">" + t("まだコメントはありません。最初のひとことを。") + "</p></div>"));
}
...
function bindCommentForm(post) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      alert(t("コメント送信に失敗しました: ") + err.message);
    } finally {
