...
const PRESETS = [
  { label: t("1 時間後"), mins:    60 },
  { label: t("1 日後"),   mins:  1440 },
  { label: t("3 日後"),   mins:  4320 },
  { label: t("1 週間後"), mins: 10080 },
];
...
export function openPollModal(initial = null) {
  ...
  host.innerHTML =
    '<div class="modal" id="poll-modal">' +
      ...
      '<div class="modal__card" role="dialog" aria-labelledby="poll-title">' +
        ("<button class=\"modal__close\" data-poll-close aria-label=\"" + t("Close") + "\">") + icon('close', { size: 18 }) + '</button>' +
        ("<h2 id=\"poll-title\" class=\"auth-form__h\">" + t("投票を作成") + "</h2>") +
        '<form class="poll-form" id="poll-form">' +
          ("<label class=\"poll-form__label\">" + t("質問")) +
            '<input name="question" type="text" maxlength="120" value="' + escape(q) + ("\" required placeholder=\"" + t("どっちにする？") + "\">") +
          '</label>' +
          ...
          '<button type="button" class="btn btn--ghost btn--sm poll-form__add" id="poll-add-option"' +
            (opts.length >= 4 ? ' disabled' : '') + (">" + t("＋ 選択肢を追加") + "</button>") +
          ("<label class=\"poll-form__label\">" + t("期限")) +
          ...
          '<div class="edit-actions">' +
            (initial ? ("<button type=\"button\" class=\"btn btn--ghost\" data-poll-delete>" + t("投票を外す") + "</button>") : '') +
            ("<button type=\"button\" class=\"btn btn--ghost\" data-poll-close>" + t("キャンセル") + "</button>") +
            ("<button type=\"submit\" class=\"btn btn--primary\">" + t("投票を追加") + "</button>") +
          '</div>' +
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (!question) { err.textContent = t("質問を入力してください"); return; }
    ...
    if (options.length < 2) { err.textContent = t("選択肢は 2 つ以上必要です"); return; }
    if (new Set(options).size !== options.length) {
      err.textContent = t("選択肢が重複しています"); return;
    }
    ...
    if (!deadlineAt || deadlineAt <= Date.now() + 30 * 1000) {
      err.textContent = t("期限はもう少し先に設定してください"); return;
    }
...
function optionRow(i, value, total) {
  return (
    '<div class="poll-form__option">' +
      ...
      '<input name="opt" type="text" maxlength="60" value="' + escape(value) + ("\" placeholder=\"" + t("選択肢 ")) + (i + 1) + '">' +
      (total > 2
        ? ("<button type=\"button\" class=\"poll-form__rm\" data-poll-remove aria-label=\"" + t("削除") + "\">×</button>")
        : '') +
    '</div>'
  );
}
...
function relabel() {
  ...
  rows.forEach((row, i) => {
    ...
    if (!input.value) input.placeholder = t("選択肢 ") + (i + 1);
  });
