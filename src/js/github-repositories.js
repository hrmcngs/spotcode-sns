...
export async function githubRepositories(token, expectedHandle, fetcher = fetch) {
  const get = async path => {
    ...
    if (response.status === 401) throw new Error(t("GitHub認証が無効です。GitHubを再認証してください。"));
    if (response.status === 403) throw new Error(t("GitHubがアクセスを拒否しました。OrganizationのOAuth許可・SSO、またはAPIの利用制限を確認してください。"));
    if (!response.ok) throw new Error(t("GitHubのリポジトリ取得に失敗しました（") + response.status + '）。');
    return response.json();
  };
  const user = await get('/user');
  if (!expectedHandle || user.login?.toLowerCase() !== expectedHandle.toLowerCase()) {
    throw new Error(t("プロフィールに連携したGitHubアカウントで再認証してください。"));
  }
