...
const orgGrant = await verifiedOrganizations('test-token', [], fakeFetch, true);
assert.deepEqual(orgGrant.organizations, [{ id: 20, login: 'allowed', role: 'admin' }]);
fixtures['/user/orgs'].push({ id: 30, login: 'not-approved' });
assert.deepEqual((await verifiedOrganizations('test-token', [], fakeFetch, true)).organizations, orgGrant.organizations);
assert.deepEqual((await authorizedRepositories('test-token', -1, orgGrant.organizations, fakeFetch)).map(r => r.id), [2]);
console.log('PASS organization account reuses an owner grant without personal identity linking, rejects member-only orgs, excludes personal repos');
