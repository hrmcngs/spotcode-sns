...
`version.sh --bump-build` はビルド番号だけを増やします。表示バージョンも変更する場合は `bash macos/version.sh 1.0.2 16` のように指定してください。`run.sh` は毎回Xcodeの差分ビルドを実行するため、バージョン番号を変えなくてもソースの変更が反映されます。`--build` は互換性のため残しており、通常起動と同じ動作です。提出用Archive・PKGはそれぞれ `build.sh archive`・`build.sh export` で作り直してください。
...
「設定 → 画面表示 → 外観」で「システムに合わせる・ライト・ダーク」を選べます。選択は端末に保存され、起動中に反映されます。初期値はシステム設定への追従です。背景・文字・区切り線・ボタン・ホバー色を外観に合わせて切り替えます。アカウントメニューを開いてもページ全体は暗転せず、メニュー外のクリックで閉じられます。左メニューは横線で区切り、カーソルを合わせると余白を含む行全体の背景色が変わります。選択中の項目には薄い背景色と左端の縦線を表示し、新規投稿・アカウント欄も同じ行形式で操作できます。
...
Debug/Releaseとも `$(AppIdentifierPrefix)$(PRODUCT_BUNDLE_IDENTIFIER)` をキーチェーン保存先に使用します。
macOSでは署名のアクセスグループを使うデータ保護キーチェーンに保存し、既存の旧形式データは読み出せた時点で移行します。
キーチェーンがロック中・アクセス拒否の場合は未ログインと決めつけず、再試行またはアプリを再度アクティブにした時に復元します。
Bundle ID・Developer Teamを変えずに署名付きの `bash macos/build.sh preview` で更新してください。
